...
const ExperienceSection = ({
  ...
  return (
    <section ref={innerRef} className="px-6 font-body">
          ...
          {experiences.map((exp, i) => (
            <div key={i} className="relative pl-10 pb-16 last:pb-0 exp-item">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-2 gap-2">
                ...
                <span className="px-4 py-1 rounded-full bg-cyan-500/10 text-cyan-400 text-sm font-mono ring-1 ring-cyan-500/20 w-fit">
                  {exp.period}
                </span>
              </div>
              <h4 className="text-xl text-white/50 mb-6 font-medium text-left">
                {exp.company}
              </h4>
              <ul className="space-y-4 text-left">
                {exp.details.map((detail: string, idx: number) => (
                  ...
                  >
                    <span className="mr-3 mt-2 w-1.5 h-1.5 bg-white/30 rounded-full shrink-0" />
                    {detail}
                  </li>
                ))}
              </ul>
